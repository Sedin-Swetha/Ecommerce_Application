...
export default function NotificationItem({ notification, onRead }: Props) {
    const content = (
        <div
            ...
            className={`flex gap-3 px-4 py-3 cursor-pointer transition hover:bg-gray-50 dark:hover:bg-gray-800 ${notification.read ? "" : "bg-primary/5 dark:bg-primary/20"
                }`}
        >
            <span
                className={`mt-1 h-2 w-2 flex-shrink-0 rounded-full ${notification.read ? "bg-transparent" : "bg-primary"
                    }`}
            />
