            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="flex h-16 items-center justify-between">
                    ...
                    <nav className="hidden md:flex items-center gap-8">
                        <Link href={ROUTES.HOME} className="font-medium text-gray-700 hover:text-blue-600 transition">Home</Link>
                        {/* <Link href="/products" className="font-medium text-gray-700 hover:text-blue-600 transition">Products</Link>
                        <Link href="#categories" className="font-medium text-gray-700 hover:text-blue-600 transition">Categories</Link> */}
                    </nav>
                    <div className="hidden md:flex items-center gap-2">
                        {user && (
                            <>
                                ...
                                <Link href="/cart" className="relative flex items-center justify-center w-10 h-10 rounded-lg text-gray-600 hover:bg-gray-100 hover:text-blue-600 transition">
                                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        {user ? (
                            <div className="relative" ref={dropdownRef}>
                                ...
                                >
                                    {user.profileImage ? (
                                        ...
                                        <img src={user.profileImage} alt={user.name} className="h-8 w-8 rounded-full object-cover ring-2 ring-blue-100" />
                                    ) : (
                                ...
                                {isDropdownOpen && (
                                    <div className="absolute right-0 mt-2 w-56 rounded-xl border border-gray-100 bg-white shadow-lg py-1 z-50">
                                        ...
                                        {user.role === UserRole.ADMIN && (
                                            <Link href="/admin" onClick={() => setIsDropdownOpen(false)} className="flex items-center gap-2 px-4 py-2 text-sm text-purple-600 hover:bg-purple-50 transition">
                            ...
                            <>
                                ...
                                <Link href={ROUTES.REGISTER} className="rounded-lg bg-blue-600 px-5 py-2 text-white hover:bg-blue-700 transition">Register</Link>
                            </>
                        )}
                    </div>
                        ...
                        {user && (
                            <>
                                ...
                                <Link href="/cart" className="relative flex items-center justify-center w-9 h-9 rounded-lg text-gray-600">
                                    ...
                                    {cartCount > 0 && (
                                        <span className="absolute top-0.5 right-0.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-blue-600 text-[9px] font-bold text-white leading-none">
                                            {cartCount}
                                        </span>
                                    )}
                                </Link>
                ...
                <div className="border-t bg-white md:hidden">
                    <div className="flex flex-col gap-1 px-4 py-3">
                        ...
                        {user && (
                            <>
                                ...
                                <Link href="/cart" onClick={() => setIsOpen(false)} className="flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 hover:bg-gray-50">
                                    ...
                                    Cart
                                    {cartCount > 0 && <span className="ml-auto flex h-5 w-5 items-center justify-center rounded-full bg-blue-600 text-[10px] font-bold text-white">{cartCount}</span>}
                                </Link>
                            </>
                        )}
                        ...
                        {user ? (
                            <>
                                {user.role === UserRole.ADMIN && (
                                    <Link href="/admin" onClick={() => setIsOpen(false)} className="rounded-lg px-3 py-2 font-medium text-purple-600 hover:bg-purple-50">Admin Dashboard</Link>
                                )}
                                ...
                                <button
                                    ...
                                    className="rounded-lg px-3 py-2 text-left font-medium text-red-500 hover:bg-red-50"
                                >
                                    Logout
                                </button>
                            </>
                            ...
                            <>
                                ...
                                <Link href="/register" onClick={() => setIsOpen(false)} className="rounded-lg bg-blue-600 px-4 py-2 text-center font-medium text-white hover:bg-blue-700">Register</Link>
                            </>
                        )}
                    </div>
                </div>
